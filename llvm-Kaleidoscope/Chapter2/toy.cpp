...
/// defined.
static std::map<char, int> BinopPrecedence; // 操作符到优先级的映射
...
static std::unique_ptr<ExprAST> ParseNumberExpr() {
  ...
  return std::move(Result); // 构造结果移动出去
}
...
static std::unique_ptr<ExprAST> ParseIdentifierExpr() {
  ...
  if (CurTok != '(') // Simple variable ref. // 判断是函数还是变量
  ...
  return std::make_unique<CallExprAST>(IdName, std::move(Args)); // 生成函数调用
}
